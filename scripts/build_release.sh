...
#
# Output:
#   dist/macro-traffic-sim-server-linux-amd64.tar.gz
#   dist/macro-traffic-sim-server-windows-amd64.zip (if cross-compile available)

set -e

VERSION="${VERSION:-$(grep '^version' Cargo.toml | head -1 | sed 's/.*"\(.*\)"/\1/')}"
DIST_DIR="dist"
BINARY_NAME="macro_traffic_sim"

echo "Building macro-traffic-sim-server v${VERSION}"
mkdir -p "${DIST_DIR}"
...
LINUX_ARCHIVE="macro-traffic-sim-server-${VERSION}-linux-amd64.tar.gz"
...
if rustup target list --installed | grep -q "${WINDOWS_TARGET}"; then
    if command -v x86_64-w64-mingw32-gcc &> /dev/null; then
        ...
        WINDOWS_ARCHIVE="macro-traffic-sim-server-${VERSION}-windows-amd64.zip"
